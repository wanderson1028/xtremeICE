...
function ScenarioCard({ scenario, onLaunch }) {
  return (
    <motion.div
      ...
      className="bg-white/5 border border-white/10 rounded-2xl p-5 hover:border-green-500/50 hover:bg-green-500/5 transition-all group cursor-pointer"
      onClick={() => onLaunch(scenario)}
    >
      <div className="flex items-start justify-between mb-3">
        <div>
          <div className="text-base font-semibold text-white group-hover:text-green-400 transition-colors">{scenario.name}</div>
          <div className="text-xs text-gray-400 mt-0.5">{scenario.category}</div>
        </div>
        <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${difficultyColor.Beginner}`}>Beginner</span>
      </div>
      <p className="text-sm text-gray-300 leading-relaxed mb-4">{scenario.description}</p>
      <div className="flex items-center gap-3 text-xs text-gray-400">
        ...
        <span className="ml-auto flex items-center gap-1 text-green-400 font-medium group-hover:gap-2 transition-all">
...
export default function SOCTraining() {
  ...
  const { data: labAssignments = [] } = useQuery({
    ...
    queryFn: () => base44.entities.UserLabAssignment.filter({
      ...
      service_key: "soc_training",
    }),
    enabled: !!currentUser?.email,
  });

  const BEGINNER_SCENARIOS = (() => {
    if (!labAssignments.length) return ALL_BEGINNER;
    const ids = new Set(labAssignments.map(a => a.lab_id));
    return ALL_BEGINNER.filter(s => ids.has(s.id));
  })();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-950 to-red-950/20 p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-6">
            <h2 className="text-lg font-semibold text-white mb-1">Select Network Environment</h2>
            <p className="text-sm text-gray-400 mb-4">Choose an enterprise template or one of your saved network designs.</p>
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-cyan-400">Enterprise Templates</span>
                <div className="flex-1 h-px bg-cyan-400/20" />
              </div>
              {templatesLoading ? (
                <div className="py-4 text-center text-gray-400 text-sm">Loading templates...</div>
              ) : (
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <span className="text-xs font-semibold uppercase tracking-wider text-gray-400">Your Designs</span>
                <div className="flex-1 h-px bg-white/10" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button onClick={() => { setSelectedNetwork({ id: "default", name: "Default Lab Environment" }); setPhase("select_scenario"); }}
                  className="p-4 bg-white/5 border border-white/10 rounded-xl hover:border-green-400/50 hover:bg-green-400/5 transition-all text-left">
                  <div className="flex items-center gap-3">
                    <Network className="h-5 w-5 text-gray-400" />
                    <div>
                      <div className="font-semibold text-sm text-white">Default Lab Environment</div>
                      <div className="text-xs text-green-400 mt-0.5">8 pre-configured virtual devices</div>
                    </div>
                  </div>
                </button>
                {networks.map(net => (
      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-950 to-red-950/20 p-6">
        <div className="max-w-5xl mx-auto">
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
            <button onClick={() => setPhase("select_network")} className="hover:text-white transition-colors">SOC Response Drills</button>
