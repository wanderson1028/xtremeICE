...
export default function TabletopExercises() {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100">
    ...
    <main className="mx-auto max-w-7xl p-5 lg:p-10">
      ...
      {tab==="profile"&&<Card className="p-6"><div className="mb-6"><h2 className="text-2xl font-bold">Required company profile</h2><p className="text-slate-400">Build a profile, create a draft from the company website, or select an existing profile before starting an exercise.</p></div><div className="mb-6 grid gap-4 lg:grid-cols-2"><div className="rounded-xl border border-cyan-500/30 bg-cyan-500/5 p-4"><p className="font-bold text-cyan-200">Create draft from company URL</p><p className="mt-1 text-xs text-slate-400">Public information creates a draft only. Security, recovery, technology, and staffing details must be verified by the user.</p><div className="mt-3 flex gap-2"><input className={inputClass} value={websiteUrl} onChange={e=>setWebsiteUrl(e.target.value)} placeholder="https://company.com"/><button disabled={urlLoading} onClick={generateFromUrl} className="shrink-0 rounded-xl bg-cyan-400 px-4 py-2.5 text-sm font-bold text-slate-950 disabled:opacity-50">{urlLoading?"Researching…":"Create draft"}</button></div></div><div className="rounded-xl border border-purple-500/30 bg-purple-500/5 p-4"><p className="font-bold text-purple-200">Demo company profiles</p><p className="mt-1 text-xs text-slate-400">Use a fictional profile to demonstrate the TTX workflow.</p><div className="mt-3 grid gap-2">{DEMO_PROFILES.map(d=><button key={d.company_name} onClick={()=>useDemoProfile(d)} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-left text-sm hover:border-purple-400"><span className="font-semibold">{d.company_name}</span><span className="ml-2 text-xs text-slate-500">{d.industry}</span></button>)}</div></div></div>{profiles.length>0&&<div className="mb-6 grid gap-3 rounded-xl border border-slate-700 bg-slate-950 p-4 md:grid-cols-[1fr_auto]"><Field label="Select an existing company profile"><select className={inputClass} value={profileId||""} onChange={e=>{const p=profiles.find(x=>x.id===e.target.value);if(p){setProfileId(p.id);setProfile(hydrateProfile(p));setSetup(s=>({...s,geography:p.primary_geography||""}));setMessage(`${p.company_name} selected. You may now start an exercise.`);}}}><option value="">Select a profile</option>{profiles.map(p=><option key={p.id} value={p.id}>{p.company_name}</option>)}</select></Field><button onClick={()=>{setProfileId(null);setProfile(blankProfile);setMessage("Enter the new company profile and save it to unlock exercises.");}} className="self-end rounded-xl border border-slate-600 px-4 py-2.5 text-sm font-semibold hover:bg-slate-800">Build new profile</button></div>}<div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
