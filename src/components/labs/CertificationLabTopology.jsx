 ...
 return <div className="shrink-0 overflow-hidden rounded-xl border border-cyan-800/40 bg-gray-950/90">
  ...
  <div className="grid md:grid-cols-[minmax(0,1fr)_220px]">
   <div className="relative min-h-[250px] overflow-hidden bg-[radial-gradient(circle_at_center,rgba(6,182,212,.09),transparent_62%)]">
    ...
    {devices.map(d=>{const isSelected=d.id===selected.id,isTarget=d.id===target;return <button key={d.id} type="button" onClick={()=>onInspect(d)} style={{left:`${d.x}%`,top:`${d.y}%`}} className={`absolute w-24 -translate-x-1/2 -translate-y-1/2 rounded-lg border p-2 text-left shadow-lg transition hover:z-20 hover:scale-105 ${isSelected?"border-cyan-300 bg-cyan-950 text-cyan-100":isTarget?"border-red-500/70 bg-red-950/60 text-red-100":"border-gray-700 bg-black/90 text-gray-300"}`} aria-label={`View ${d.name} details`}>
     {isTarget&&!isSelected&&<span className="absolute -right-1 -top-1 h-2.5 w-2.5 animate-ping rounded-full bg-red-400"/>}
     <div className="flex items-center justify-between"><InfrastructureDeviceIcon type={d.type} className="h-6 w-6"/><span className="h-1.5 w-1.5 rounded-full bg-emerald-400"/></div>
