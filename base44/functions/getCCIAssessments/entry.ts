...
Deno.serve(async(req)=>{
 try{
  ...
  if(user.role==="admin"){
   ...
   ratings=requested?await base44.asServiceRole.entities.CCISecurityRating.filter({organization_id:requested}):await base44.asServiceRole.entities.CCISecurityRating.list("-calculated_at",250);
  }else{
   ...
   ratings=await base44.asServiceRole.entities.CCISecurityRating.filter({organization_id:orgId});
  }
  ...
  return Response.json({success:true,assessments:completed,ratings:ratings||[],organizations:Array.from(new Map(completed.map((r:any)=>[r.organization_id,{id:r.organization_id,name:r.business_name||"Unnamed organization"}])).values())});
