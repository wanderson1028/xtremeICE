...
const BASELINE=650, MIN=300, MAX=850, VERSION="CFRS-ASSESS-2026.10";
...
Deno.serve(async(req)=>{
 try{
  ...
  const priorRows=(await base44.asServiceRole.entities.CCIAssessment.filter({organization_id:orgId})).filter((row:any)=>row.status==="completed"&&row.report_fingerprint!==reportFingerprint).sort((a:any,b:any)=>evidenceDate(b).getTime()-evidenceDate(a).getTime());
  ...
  const trendAdjustment=priorSamples.length?Math.round(clamp((latestScore-priorAverage)*.15,-20,20)):0;
