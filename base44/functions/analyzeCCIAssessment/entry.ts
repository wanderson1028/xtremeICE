...
Deno.serve(async(req)=>{
 try{
  ...
  const movement=finalScore-BASELINE;
  const rating=band(finalScore);
  const pressureDrivers=[
   ...(vAdj<0?[`Vulnerability exposure reduced the rating by ${Math.abs(vAdj)} points.`]:[]),
   ...(pAdj<0?[`Penetration-test resilience reduced the rating by ${Math.abs(pAdj)} points.`]:[]),
   ...(validatedAttackPenalty?[`Verified attack outcomes reduced the rating by ${validatedAttackPenalty} points.`]:[]),
   ...(repeatFindingPenalty?[`Repeated unresolved issues reduced the rating by ${repeatFindingPenalty} points.`]:[]),
   ...(exposureConcentrationPenalty?[`Concentrated high-risk exposure reduced the rating by ${exposureConcentrationPenalty} points.`]:[])
  ];
  const strengthDrivers=[
   ...(vAdj>0?[`Vulnerability posture contributed ${vAdj} points above the baseline.`]:[]),
   ...(pAdj>0?[`Penetration-test resilience contributed ${pAdj} points above the baseline.`]:[]),
   ...(remediationAdjustment?[`Verified remediation contributed ${remediationAdjustment} points.`]:[])
  ];
