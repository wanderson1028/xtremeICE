...
Deno.serve(async(req)=>{
 try{
  ...
  const vAdj=vulnerabilityAssessed?Math.round(postureAdjustment(Number(vulnerabilityScore),150,300)):0, pAdj=Math.round(postureAdjustment(pentestScore,200,350));
  ...
  const strengthDrivers=[
   ...(vAdj>0?[`Vulnerability posture contributed a positive ${vAdj}-point resilience adjustment.`]:[]),
   ...(pAdj>0?[`Penetration-test resilience contributed a positive ${pAdj}-point adjustment.`]:[]),
  ...
  if(current){
   for(const previous of allOrganizationRows.filter((row:any)=>row.is_current_rating===true&&row.id!==created.id))await base44.asServiceRole.entities.CCIAssessment.update(previous.id,{is_current_rating:false});
